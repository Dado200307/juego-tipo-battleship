...
client = OpenAI(
  api_key = "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",
)
        ...
        while True:
            ...
            if self.tablero.realizar_movimiento(columna, self.jugador_actual):
                ...
                if self.tablero.verificar_ganador(self.jugador_actual):
                    ...
                    nombre_ganador = self.jugador_2
                    if self.jugador_actual == self.jugador_2_simbolo:
                        nombre_ganador = self.jugador_1
                        
                    chat_completion = client.chat.completions.create(
                        messages = [
                            {
                                "role": "user",
                                "content": f"generame un mensaje para felicitar a un jugador llamado {nombre_ganador}, y dile de forma ironica al otro jugador que perdio",
                            }
                        ],
                        model = "gpt-3.5-turbo",
                    )
